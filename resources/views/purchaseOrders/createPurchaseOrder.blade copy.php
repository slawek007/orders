...
@section('purchaseData')
    ...
    <form action="{{ action('PurchaseNumberController@create') }}">
        <button type="submit" class="PurchaseNumberSubmit btn btn-success">rezerwój nr zamówienia</button>
    </form>
    <form action="{{ action('PurchaseNumberController@destroy', ['id' => '15']) }}" method="post">
        {!! method_field('delete') !!}
        <button type="submit" class="PurchaseNumberDestroy btn btn-danger">zwolnij nr zamówienia</button>
    </form>
@endsection
...
@endpush
@push('endSiteScripts')
    <script type="text/javascript" src="{{ asset('js/purchaseOrderNumberGenerator.js') }}"></script>
